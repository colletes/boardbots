...
"""
content = content.replace('<!-- RESULT MODAL -->', game_end_html + '\n</div>\n\n' + calc_summary_html + '\n<!-- RESULT MODAL -->')
...
"""
content = re.sub(r'function startGame\(\).*?function selectColor\(c\)', js_vars + '\n\nfunction selectColor(c)', content, flags=re.DOTALL)
content = re.sub(r'function resetGame\(\) \{.*?\}\n\}', '', content, flags=re.DOTALL)

# Remove the old originalStartGame block at the bottom
content = re.sub(r'const originalStartGame = startGame;.*?\}', '', content, flags=re.DOTALL)
