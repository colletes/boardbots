...
html = html.replace("if (state.military <= -9) {", "if (false) {") # Disable player win via military
...
# 2. Fix the Fortress UI (remove the "Neutro" / "Você avança" logic since it's just a 0-7 counter now)
...
# 3. Restructure UI: Landmarks above Card Area, Trilha/Fichas always open below it.
# We will inject a new Landmark panel before the card area, and move Trilha/Fichas below it.
# First, remove the old buttons "Rolar Dado", "Landmarks", "Trilha/Fichas" from action-row.
...
# The "🎲 Sortear Ficha" button in the Race chips header will instead trigger the dice-box library.
# We will rewrite executeDraftRoll later.

# Remove the diceResult area from old place
...
# Inject the new Landmark Panel ABOVE the decision card
landmark_panel = """
...
old_card_area_start = """<!-- Fallback Scan Direction -->"""
html = html.replace("<!-- Fallback Scan Direction -->", landmark_panel + "\n        <!-- Fallback Scan Direction -->")


# Inject the Trilha/Fichas panel BELOW the decision card
trilha_panel = """
...
# Replace the Action Controls section so it goes below it
...
# Add Dice Box container to the HTML
dice_container = """
  <!-- Dice Box Container -->
...
"""
html = html.replace("<body>", "<body>" + dice_container)

# Add Dice Box initialization to JS
dice_js = """
    import DiceBox from '../assets/dice-box/dice-box.es.min.js';
    let diceBox;
    ...
    async function initDiceBox() {
      ...
      diceBox = new DiceBox({
        ...
        theme: 'default',
        themeColor: '#475569', // Default grey, will update dynamically
        scale: 6
      });
      await diceBox.init();
    ...
    window.rollDiceBox = rollDiceBox;

    function botBuyLandmark() {
      // Deduct cost and add log
      let cost = 7;
      if (state.currentAge === '2') cost = 5;
      if (state.currentAge === '3') cost = 2;
      cost += state.military; // military is now fortresses
...
# Fix generic logs
html = html.replace("function addLog(msg) {", """function addLog(msg) {
      if (msg.type === 'generic') {
